...
После некоторых операций (`create`, `destroy`, `restart`, `start`, `stop`, `update`) скрипт умеет запускать
произвольные скрипты (хуки) для каких-либо до-настроек (например, регистрация DNS-записи, настройка балансировщика и т.д.).
Путь до хук-скриптов указывается параметром `local_hook_path`, в котором можно указать как путь до конкретного исполняемого
скрипта, так и до директории со скриптами, которые будут выполнены в алфавитном порядке.
...
| **TYPE** | Тип операции, из которой хук-скрипт был вызван (сейчас поддерживаются: `create`, `destroy`, `restart`, `start`, `stop`, `update`) | |
