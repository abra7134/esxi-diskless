...
`vm_timezone`, `vm_vnc_port`. Все параметры обновляются на "горячую", т.е. меняют свои значения сразу же, но только
в конфигурации гипервизора. Поэтому, чтобы обновленные значения подхватились самими виртуальными машинами необходима
их перезагрузка (это касается параметров `vm_dns_servers` и `vm_timezone`).

При обновлении `vm_vnc_port` параметра также обновляется значение и `vm_vnc_password` параметра.
Раздельное обновление только порта или только пароля для VNC-доступа на данный момент не поддерживается.
...
| **vm_vnc_port** | Номер порта для VNC-доступа к виртуальной машины (помимо номера может принимать значения: `auto` - для автоматического выбора порта, `off` - для отключения VNC-доступа) | off |
| **vm_vnc_password** | Пароль для VNC-доступа к виртуальной машине | |
...
Каждая команда заканчивается каким-либо результатом, в терминах скрипта - статусом. Статус может быть одиночным
или составным, т.е. содержать в себе несколько статусов. Одиночный статус или самый первый из составного называется
основным, остальные - вспомогательными. Статусы разделяются наклонной чертой `/`, например:

```
CREATED/PINGED/VNC ENABLED (Runned on 'xenial-bro-211227-63f26add.iso')
```

В круглых скобках указывается расшифровка статусов, при необходимости.

Ниже приведены таблицы со всеми возможными основными и вспомогательным статусами с уточнением в каких именно
командах конкретный статус может возникать:

| Основной статус | Описание значения | Может возникнуть в командах |
| ------ | ----------------- | --------------------------- |
| ABORTED | Операция была прервана по серъезной ошибке или по требованию пользователя (требуется реакция оператора, т.к. может быть промежуточное состояние) | любых |
| NOT PROCESSED | Виртуальная машина не была обработана из-за прерывания обработки (встречается вместе с ABORTED) | любых |
| SKIPPED ESXI | Обработка всех виртуальных машин была пропущена из-за проблемы на гипервизоре | любых |
| SKIPPED VM | Обработка виртуальной машины была пропущена из-за какой-либо ошибки | любых |
| CREATED | Виртуальная машина создана | create, create с опцией `-d` |
| DESTROYED | Виртуальная машина уничтожена | create с опцией `-d`, destroy |
| REBOOTED | Виртуальная машины перезагружена | create с опцией `-d`, reboot |
| RECREATED | Виртуальная машина пересоздана | create с опцией `-f` |
| STARTED | Виртуальная машина включена | start |
| STATUS REQUESTED | Получен статус питания виртуальной машины | status |
| STOPPED | Виртуальная машина выключена | stop |
| UPDATED | Обновлен параметр виртуальной машины | update |
| UPDATE NOT REQUIRED | Обновление параметра виртуальной машины не требуется (уже имеет нужное значение) | update |

| Вспомогательный статус | Описание значения | Может возникнуть в командах |
| ---------------------- | ----------------- | --------------------------- |
| NO PINGING | Виртуальная машина не доступна по сети | create, reboot, start |
| PINGING | Виртуальная машина доступна по сети | create, reboot, start |
| VNC ENABLED | VNC-доступ до виртуальной машины организован | create |
| VNC FAILED | Проблема при организации VNC-доступа к виртуальной машине | create |
| VNC UNKNOWN | Не удалось распарсить URL-строчку с VNC-доступом к виртуальной машине | create |

При операции миграции (команда `create` с опцией `-d`) в статусе вместо одной виртуальной машины появятся две:
одна созданная и одна уничтоженная:

![](screenshots/control_vm_esxi-create-migrate.png)
...
Такая же схема будет применяться и для уничтоженной виртуальной машины при "миграции" (команда `create` с опцией `-d`).
