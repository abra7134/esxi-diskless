...
Взаимодействие с гипервизорами происходит по `SSH`-протоколу и посредством `API` (через программу `govc`). Взаимодействие
посредством `API` потребовалось команде `update` для горячей замены `ISO`-образов. В будущем планируется использовать
только один вид транспорта.
...
Команда отображает 'виденье' конфигурационного файла скриптом.
Для уточнения параметров, а также опций команды достаточно просто её запустить:

![](screenshots/control_vm_esxi-ls.png)
...
Команда для создания и запуска виртуальных машин.
Для уточнения параметров, а также опций команды достаточно просто её запустить:

![](screenshots/control_vm_esxi-create.png)
...
![](screenshots/control_vm_esxi-destroy.png)
...
![](screenshots/control_vm_esxi-reboot.png)
...
Команда показывает состояние виртуальных машин в конфигурационном файле и на гипервизорах, а также разницу настроек
в них. Для уточнения параметров, а также опций команды достаточно просто её запустить:

![](screenshots/control_vm_esxi-show.png)
...
Команда для обновления настроек виртуальных машин.
Для уточнения параметров, а также опций команды достаточно просто её запустить:

![](screenshots/control_vm_esxi-update.png)
...
На данный момент команда умеет обновлять настройки только для одного параметра - `local_iso_path`. Т.е. умеет
только заменять `ISO`-образ для виртуальных машин, причем делает это на горячую, т.е. чтобы он стал актуальным,
необходимо перезагружать виртуальную машину.
...
Команда для предварительной загрузки `ISO`-образов на гипервизоры.
Для уточнения параметров, а также опций команды достаточно просто её запустить:

![](screenshots/control_vm_esxi-upload.png)
