...
```
сборка базового слоя ОС (при необходимости)
-> распаковка архива (артефакта) с преподготовленным базовым слоем ОС
...
* **ls** - Команда для вывода списка сборок описанных в конфигурационном файле;

##### Команда `ls`

Параметров у команды нет, поэтому запуск простой:

![](screenshots/build_iso_images-ls.png)
...
![](screenshots/build_iso_images-build.png)
...
Конфигурация сборок указывается в одноимённом INI-файле. Там перечисляются все сборки, которые скрипт может собрать.
Вот пример конфигурации из `.ini.example`:
...
при изменении их не требуется пересборка базового слоя, а требуется только пеерсборка конечного `ISO`-образа.
...
* **repo_url** - Путь до `GIT`-репозитария (параметр из конфигурационного файла);
* **repo_checkout** - Ревизия/ветка на которую надо переключиться после клонирования `GIT`-репозитария (параметр из конфигурационного файла);
* **repo_clone_into** - Папка в которую надо склонировать `GIT`-репозитарий (параметр из конфигурационного файла);
* **repo_head_short_hash** - Хеш-сумма среза (коммита) после переключения на `repo_checkout` ревизию/ветку;
* **run_from_repo** - Путь до скрипта деплоя в `GIT`-репозитарии (параметр из конфигурационного файла);
