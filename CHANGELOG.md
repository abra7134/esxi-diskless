...
v4.220427
- Исправлена установка пароля для VNC-доступа на ESXi v6.7+
- Добавлено предупреждение, что обновление параметра `vm_vnc_port` для ESXi v6.7+ носит отложенное действие
  и необходима перезагрузка виртуальных машин

v4.220418
- Добавлена поддержка управления VNC-доступом к виртуальным машинам через новые параметры `vm_vnc_port` и
  `vm_vnc_password`. В качестве порта могут быть указаны значения `auto`, `off` и конкретный номер порта.
- Добавлено отображение в статусе исходного `ISO`-образа и/или `VMDK`-шаблона (Runned on 'example.iso') на базе
  которого/ых была пересоздана виртуальная машина (команда `create` с опцией `-f`): ранее отображалось только
  для вновь созданных виртуальных машин
- Исправлен счетчик созданных/пропущенных виртуальных машин в команде `create` (ошибка появилась в v4.220407)

v4.220407
- При операции миграции виртуальной машины (команда `create` с опцией `-d`) теперь будут генерироваться два
  статуса операции вместо одного: один для старой, другой для новой виртуальной машины и, соответственно, теперь
  будут вызываться два хука
- Исправлен вызов хуков для операций, которые были прерваны по какой-либо серъезной ошибке (статус `ABORTED`)
  (ошибка появилась в v4.220314)

v4.220331
- Исправлено отображение счетчиков оставшихся попыток в разных операциях
- Секция `[vm_list]` в конфигурационном файле стала не обязательной, это может потребоваться для быстрых оперативных
  операций (перезагрузка, уничтожение, старт, стоп, получение статуса) с существующими виртуальными машинами
  при использовании `<esxi_name>/<vm_name>` нотации указания аргументов команды
- Теперь хук-скрипты также будут запускаться и для случаев отсутствия виртуальной машины на гипервизоре
  (для команд `destroy`, `reboot`, `start`, `status`, `stop`)

v4.220324
- Для команды 'create' добавлено создание специального маркер-файла '.safe_to_remove' в папке виртуальной
  машины, для автоматизированного удаления папки, в случае если она осталась после прошлой неудачной операции
  (ранее скрипт пропускал создание по ошибке: The directory is already exist on hypervisor)

v4.220321
- Добавлена новая команда `status` для получения статуса питания виртуальных машин

v4.220317
- Добавлены новые команды `start` и `stop` для включения и выключения виртуальных машин, соответственно

v4.220314
- Теперь хук-скрипты запускаются всегда в независимости от статуса операции (ранее запускались только
  при удачном исходе). Статус операции и её детальное описание теперь передаются хук-скриптам в новых
  переменных окружения `STATUS` и `STATUS_DESCRIPTION`
- Добавлены новые статусы операций `REBOOTED/PINGING` и `REBOOTED/NO PINGING` (ранее был только `REBOOTED`)
- Добавлена поддержка вызова хук-скриптов для `update` команды. Добавлен новый статус операции
  `UDPATE NOT REQUIRED`

v4.220311
- Теперь уничтожение виртуальных машин с `hdd`-диском запрещено по умолчанию
  (это не безопасная операция, поскольку также уничтожается и сам `hdd`-диск),
  однако добавлена новая опция `-ed` для разрешения этой операции
- Удален лишний подсчет контрольной суммы образа/шаблона на локальной машине для случаев,
  когда образ/шаблон уже существует на гипервизоре, а принудительная проверка не требуется
- Исправлена проверка значений `local_hook_path`, `local_iso_path`, `local_vmdk_path`
  параметров (ошибка появилась в v4.220303)

v4.220303
- Добавлена поддержка `hdd`-дисков через новые параметры `local_vmdk_path`, `vm_hdd_gb` и `vm_hdd_devtype`.
  В них указываются, соответственно: путь до шаблона, из которого и будет подготовлен образ `hdd`-диска,
  размер результирующего `hdd`-диска и его тип (поддерживаются значения `pvscsi`, `lsasas1068`, `lsilogic`)
- Добавлена поддержка указания пустых значений для `local_iso_path` и `local_vmdk_path` параметров, таким
  образом теперь скрипт умеет работать с 4мя типами виртуальных машин: дисковые, бездисковые, бездисковые
  с диском для хранения каких-либо данных, сетевые (загрузка только по сети)
- Улучшено обнаружение ситуаций, когда удалять ненужный ISO-образ не безопасно (в случаях
  когда для какой-либо виртуальной машины не были получены VMX-параметры)
- Восстановлено удаление ненужных ISO-образов при уничтожении (команда `destroy`) виртуальных машин
  (ошибка появилась в v3.210823)
- Улучшен парсер VMX-конфигураций: теперь пропускает пустые строки

v3.220225
- Для параметра `local_hook_path` добавлена поддержка указания директории, в которой будут
  запущены все исполняемые файлы в алфавитном порядке
- Исправлено не срабатывание хука при операциях `destroy` и `reboot` (ошибка появилась в v3.210816)

v3.211222
- Добавлено принудительное обновление параметров виртуальной машины в VMX-файле в команде `update`,
  для обхода бага в некоторых сборках ESXi, когда при обновлении значения параметр просто удаляется
  из VMX-файла
- Исправлено некорректное отображение различий для `local_iso_path` параметра в `show` комманде,
  если в качестве значения указан полный путь, а не локальный (ошибка появилась в v2.210528)
- Исправлена ошибка 'sed: unrecognized option '--silent'' при определении количества оперативной
  памяти на старых версиях гипервизора (<6.5) (ошибка появилась в v3.211118)

v3.211220
- Добавлена поддержка обновления 'vm_dns_servers' и 'vm_timezone' параметров виртуальных машин

v3.211118
- Добавлено автоматическое включение менеджера автостарта виртуальных машин в гипервизоре,
  если он отключен, а новая опция '-da' команды 'create' возвращает старое поведение:
  предупреждение о том, что необходимо включить и настроить менеджер автостарта вручную
- Добавлена проверка достаточного количества свободной оперативной памяти на гипервизоре при
...
v3.210907
- Исправлен парсинг `host-based` CD-ROM, приводящий к некорректному отображению состояния
  в команде `show` (ошибка появилась в v3.210905)

v3.210905
- Доработан парсер VMX-параметров: добавлена поддержка `host-based` CD-ROM

v3.210904
- Доработан парсер списка файловых систем на гипервизоре на поддержку не полных UUID
  (встречаются, например, при монтировании NFS-каталогов)
