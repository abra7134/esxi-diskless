...
v4.??????
- Добавлена поддержка `hdd`-дисков через новый параметр `local_vmdk_path`, в котором указывается
  путь до шаблона, из которого и будет подготовлен образ `hdd`-диска
